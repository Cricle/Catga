...
| 操作类型 | 平均耗时 | 内存分配 | 吞吐量 |
|---------|---------|---------|--------|
| **命令处理** (单次) | ~10-20 μs | ~10 KB | 50K-100K ops/s |
| **查询处理** (单次) | ~10-20 μs | ~10 KB | 50K-100K ops/s |
| **事件发布** (单次) | ~400 ns | ~200 B | 2M+ ops/s |
| **批量命令** (100) | ~1-2 ms | ~1 MB | 50K-60K ops/s |
| **批量事件** (100) | ~40 μs | ~20 KB | 2M+ ops/s |
...
| **支付处理** | ~15 μs | 支付命令处理 |
| **订单查询** | ~12 μs | 单个订单查询 |
| **完整流程** | ~35 μs | 订单创建 + 事件发布 |
| **电商场景** | ~50 μs | 订单 + 支付 + 查询 |
| **高吞吐批量** | ~1.5 ms | 100个订单批量处理 |

### 并发性能

| 并发级别 | 吞吐量 | 内存分配 |
|---------|--------|---------|
| **10 并发** | 80K+ ops/s | 低 GC 压力 |
| **100 并发** | 100K+ ops/s | 中等 GC 压力 |
| **1000 并发** | 120K+ ops/s | 可控 GC 压力 |
