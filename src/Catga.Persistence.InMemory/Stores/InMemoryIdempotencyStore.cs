...
public sealed class InMemoryIdempotencyStore(IMessageSerializer serializer, TimeSpan? retention = null) : IIdempotencyStore
{
    private readonly ConcurrentDictionary<long, (DateTime At, byte[]? Data)> _store = new();
    private readonly TimeSpan _retention = retention ?? TimeSpan.FromHours(24);
