    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(handlerProvider, static (spc, handlers) =>
        {
            if (handlers.Length == 0)
            {
                ...
                // Still emit bootstrap to call the (empty) registration for consistency
                spc.AddSource("CatgaGenerated.Bootstrap.Handlers.Init.g.cs", SourceText.From(GenerateBootstrap(), Encoding.UTF8));
                return;
            }
...
";
    ...
    private static string GenerateBootstrap()
    {
        ...
        sb.AppendLine("        {");
        sb.AppendLine("            global::Catga.Generated.GeneratedBootstrapRegistry.Register(static services => global::Catga.DependencyInjection.CatgaGeneratedHandlerRegistrations.AddGeneratedHandlers(services));");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine("}");
        return sb.ToString();
