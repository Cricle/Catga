...
public class EndpointGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // Find assembly attribute [assembly: CatgaEndpoints]
        var assemblyAttrProvider = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                "Catga.CatgaEndpointsAttribute",
                predicate: static (node, _) => node is CompilationUnitSyntax,
                transform: static (ctx, _) => GetEndpointConfig(ctx))
        ...
        var combined = assemblyAttrProvider.Combine(handlerProvider);
        context.RegisterSourceOutput(combined, static (spc, data) =>
        {
            var (config, handlers) = data;
            if (config is null || handlers.Length == 0) return;

            var source = GenerateEndpointExtensions(config, handlers.Where(h => h is not null).ToList()!);
    ...
    private static string GenerateEndpointExtensions(EndpointConfig config, List<HandlerEndpointInfo> handlers)
    {
        ...
        sb.AppendLine("using Catga.AspNetCore;");
        sb.AppendLine();
