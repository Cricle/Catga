...
public class WhileLoopTests
{
    ...
    [Fact]
    public async Task While_RedisStorage_ShouldRecover()
    {
        // Arrange
        var mediator = Substitute.For<ICatgaMediator>();
        ...
        var state = new WhileTestState { FlowId = "while-recovery-redis", Counter = 0 };
        SetupMediator(mediator);
        ...
        try
        {
            ...
            // Simulate crash and recovery
            var state2 = await store.GetAsync<WhileTestState>("while-recovery-redis");
            var executor2 = new DslFlowExecutor<WhileTestState, SimpleWhileFlow>(mediator, store, config);
            var result2 = await executor2.RunAsync(state2);
    ...
    [Fact]
    public async Task While_NatsStorage_ShouldRecover()
    {
        // Arrange
        var mediator = Substitute.For<ICatgaMediator>();
        ...
        var state = new WhileTestState { FlowId = "while-recovery-nats", Counter = 0 };
        SetupMediator(mediator);
        ...
        try
        {
            ...
            // Simulate crash and recovery
            var state2 = await store.GetAsync<WhileTestState>("while-recovery-nats");
            var executor2 = new DslFlowExecutor<WhileTestState, SimpleWhileFlow>(mediator, store, config);
            var result2 = await executor2.RunAsync(state2);
